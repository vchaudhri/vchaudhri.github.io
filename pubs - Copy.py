...
def generate_html_from_csv(csv_file, output_html):
    ...
    with open(csv_file, newline='', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            <li>
                <h2>{row['Name']}</h2>
                <p><strong>Published On:</strong> {row['Published']}</p>
                <p><strong>Description:</strong> {row['Description']}</p>
                <p><strong>Publisher:</strong> {row['Publisher']}</p>
                <p><strong>URL:</strong> <a href="{row['Url']}" target="_blank">Link</a></p>
            </li>
            """
...
generate_html_from_csv('publications.csv', 'publication.html')
